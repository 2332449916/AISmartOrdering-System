...
# from smart_diancan.tools.test_llm_invoke import llm
...
def  call_llm(query:str, system_instruction:str)->str:
    ...
    # 3.1PromptTemplate或者ChatPromptTemplate对象 有且只有两种方式（①实例化ChatPromptTemplate() ②调用类方法ChatPromptTemplate.From_message())
    ...
    # resp = chain.invoke({"system_instruction": system_instruction, "query": query})
    response = chain.invoke({"system_instruction": system_instruction,
                             "query": query})
...
# def ok_llm(query:str, system_instruction:str)->str:
#     """
#     通用LLM处理
#     args1:问题
#     args2:摸个业务对应的提示词是什么
...
#     # chain= chat_prompt_template | llm
#     chain = chat_prompt_template | llm
#     # 5.执行链（分别执行链上的每一个组件）Runnable(可运行)---invoke()[ 1.提示词模板对象 2.llm实例 3.链 4.工具 Agent] # 执行模型（）（给服务端发请求）
#
#     # 5.执行链（分别执行链上的每一个组件） Runnable(可运行)---invoke()[ 1.提示词模版对象 2.llm实例 3.链 4.工具 Agent]  # 执行模型（给服务端发送请求）
#     resp= chain.invoke({"system_instruction":system_instruction,"query":query})
